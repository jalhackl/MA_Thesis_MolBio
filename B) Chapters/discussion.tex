...
The ESOL dataset contains a lot of fatty acids and related compounds and it is known that the target property, solubility, is highly negatively correlated with the length of fatty acids. This explains why some earlier comparisons with fingerprints, including only the bitwise versions, claimed GNNs to be superior.
...
The effect of frozen layers has been analyzed checking Tanimoto similarity and layer outputs. Interestingly, for most datasets, frozen GIN layers provide much worse results when coupled with batch normalization.


A reasonable explanation is that Batch normalization reduces the variability and so gives rise to the counter-intuitive result that for frozen layers it worsens performance (, i.e. a typical case of oversmoothing).
