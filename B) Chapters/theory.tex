...
Molecular fingerprint are used in cheminformatics to represent the structure of molecules for various purposes, such as similarity searching and machine learning in drug discovery.
A hash function is used (see also methods section).
and daylight fingerprints, other descriptors directly incorporate physicochemical properties, e.g. the PaDel-descriptors \cite{Yap.2011}.
\cite{Capecchi.2020}
...
Using the ECFP-fingerprints based on the Morgan algorithm, one has to be cautios that the number indicates the diameter, i.e. ECFP4 has a radius of 2, ECFP6 of 3.

A hash function is applied to create for each unique substructure a hash value which serves as an almost - except in the case of collisions - unique identifier. A binary feature vector of fixed length is initialized with zeros and the substructure information is stored by setting for each hash value the according bit in the vector to one, i.e. each entry in the vector tells whether a particular substructure is present or absent.
...
One can also use count-Fingerprints. In this case, also the frequency of each unique substructure in a molecule is counted and the according value stored in the vector as integer.
...
Most Graph Neural Networks rely on message passing. In this step, each node of the graph passes its feature information to its neighbors and aggregates the information received from them. This process is repeated across multiple layers, allowing each node to gather information from increasingly distant nodes in the graph.

After the graph layer operations, the output has to be arranged in such a way that it can be used for the final prediction.
...
In contrast to Morgan fingerprints, which either provide binary or integer vectors, the GNN architectures usually output a real-valued vector.
...
One of the generally most commonly used GNN architectures is the Graph Convolutional Neural Network. It can be seen as a generalization of CNNs; Whereas the latter can only process input of fixed size, GCNs act directly on the graph representation. As for most graph-based representations, the graph information consists of the adjacency matrix (which in the case of molecule property prediction usually encodes the bonds between atoms) and a feature matrix (which encompasses the physicochemical features of the atoms, e.g. atom type).
...
, where W denotes the learnable weight matrix, A the adjacency matrix and D the diagonal degree matrix. \begin{math}\sigma\end{math} corresponds to an activation function, e.g. ReLU.
...
Optional: If edge features \( e_{uv} \) are processed before aggregation, the edge transformation can be expressed as:

\[
e'_{uv} = \text{EdgeMLP}^{(k)}(e_{uv})
\]

Then, the node update formula becomes:

\[
h_v^{(k+1)} = \text{MLP}^{(k)} \left( (1 + \epsilon^{(k)}) \cdot h_v^{(k)} + \sum_{u \in \mathcal{N}(v)} \text{ReLU} \left( h_u^{(k)} + e'_{uv} \right) \right)
\]

Where \( e'_{uv} \) is the transformed edge feature vector using an additional multi-layer perceptron \(\text{EdgeMLP}^{(k)}\).


\subsubsection{GAT}

Graph Attention Networks incorporate information about the neighbourhood of each node by applying an attention mechanism \cite{Velickovic.30.10.2017}.


Attention scores are computed by introducing a learnable weight vector:
...
This attention mechanism should allow to capture long-range dependencies (i.e. far distant nodes) and focus on the most relevant interactions.
...
In \cite{Brody.30.05.2021b}, a revised version of the GAT is proposed.
...
A revised version of GAT, introduced in \cite{Brody.30.05.2021b}, further improves its performance by addressing some of its limitations.
...
In contrast to GCNs and GINs, it also incorporates edge information directly. It extends the idea of the Neural Fingerprint with an attention mechanism similar to that used in GAT.
...
Since all subgraphs are copied to the GPU memory, this approach is only feasible for rather small graphs (as it is typical for molecular graphs).
...
MPNN \cite{Gilmer.05.04.2017}.
Different architectures are termed MPNN, the central property is a message passing and a readout step.
...
A message passing algorithm also using bond information, D-MPNN, was proposed in \cite{Yang.2019}.

and a motif extraction step \cite{Zang.2023}
...
Similarly, in \cite{vanTilborg.2022}, all models were trained for 300 epochs and the early stopping patience was set to ten epochs, which arguably is a very low number in this setting (given the fact that most datasets are not very big), possibly excluding good solutions.
...
Firstly, the limits of expressivity, i.e. there are graphs which cannot be distinguished by GNNs working with the message passing paradigm, have to be mentioned.

However, since in most molecular property prediction tasks the central goal is not to have a unique mapping of each molecule, but a representation which allows an accurate prediction of the objective value. (In principle, for a binary classification task it would be sufficient to map all input graphs to two different output vectors.)
...
Over-smoothing occurs predominantly in models with stacked graph layers. After several layers, node embeddings tend to converge to similar values, which diminishes the ability of the model to differentiate between nodes and often leads to degraded performance.
...
\cite{CortesCiriano.2019} used Morgan Fingerprints with a comparatively small size (128) as input for DNN and RF ensembles, reporting that larger fingerprint sizes did only marginally improve results. In this publication, another technique called snapshot ensemble learning is introduced \cite{CortesCiriano.2019}. The training process is divided into cycles. After each converged cycle the learning rate is set back to its original value.
